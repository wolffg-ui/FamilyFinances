...
async function merge(source, destination, ignoreFilter, sourceRoot) {
  ...
  if (ignoreFilter) {
    ...
    if (relPath && !ignoreFilter(relPath)) {
      await (0, import_fs_extra2.remove)(source);
      return;
    }
  }
  const destStat = await (0, import_fs_extra2.stat)(destination).catch(
    (err) => err
  );
  if ((0, import_error_utils.isErrnoException)(destStat)) {
    if (destStat.code === "ENOENT") {
      await (0, import_fs_extra2.move)(source, destination);
      return;
    }
    throw destStat;
  } else if (destStat.isDirectory()) {
    const contents = await (0, import_fs_extra2.readdir)(
      source
    ).catch((err) => err);
    if ((0, import_error_utils.isErrnoException)(contents)) {
      ...
      await Promise.all(
        contents.map(
          (name) => merge(join(source, name), join(destination, name), ignoreFilter, root)
        )
      );
      await (0, import_fs_extra2.rmdir)(source);
      return;
    }
  }
  await (0, import_fs_extra2.remove)(destination);
  await (0, import_fs_extra2.move)(source, destination);
}
...
var { normalize } = posix;
var OUTPUT_DIR = join2(VERCEL_DIR, "output");
...
function getServiceOutputDir(outputDir, service) {
  return join2(outputDir, "services", service.name);
}
  ...
  for (const [path2, output] of Object.entries(buildResult.output)) {
    ...
    if (isContainerImage(output)) {
      ...
      if (fallback) {
        ...
        const fallbackPath = join2(outputDir, "functions", fallbackName);
        let usedHardLink = false;
        if ("fsPath" in fallback) {
          try {
            await import_fs_extra3.default.link(fallback.fsPath, fallbackPath);
            usedHardLink = true;
          } catch (_) {
          }
        }
        if (!usedHardLink) {
          ...
          await (0, import_promisepipe2.default)(
            stream,
            import_fs_extra3.default.createWriteStream(fallbackPath, { mode: fallback.mode })
          );
        }
      ...
      const prerenderConfigPath = join2(
        outputDir,
        "functions",
      ...
      await import_fs_extra3.default.writeJSON(prerenderConfigPath, prerenderConfig, { spaces: 2 });
...
async function writeBuildResultV3(args) {
  ...
  const routesJsonPath = join2(workPath, ".vercel", "routes.json");
  if (isBackendBuilder(build) || build.use === "@vercel/python") {
    if ((0, import_fs_extra3.existsSync)(routesJsonPath)) {
      try {
        ...
        const routesJson = await import_fs_extra3.default.readJSON(routesJsonPath);
    ...
    if (isBackendBuilder(build) && isExperimentalBackendsEnabled() && "routes" in buildResult) {
      return writeBuildResultV2({
        repoRootPath,
        outputDir,
        buildResult,
  ...
  const path2 = service && isExperimentalServiceV2(service) ? "index" : service && typeof service.runtime === "string" ? stripDuplicateSlashes((0, import_fs_detectors.getInternalServiceFunctionPath)(service.name)) : stripDuplicateSlashes(
...
async function writeStaticFile(outputDir, file, path2, overrides, cleanUrls = false) {
  ...
  const dest = join2(outputDir, "static", fsPath);
  await import_fs_extra3.default.mkdirp(dirname(dest));
  if ("fsPath" in file) {
    if (resolve2(file.fsPath) === resolve2(dest)) {
      return;
    }
    try {
      return await import_fs_extra3.default.link(file.fsPath, dest);
    } catch (_) {
    }
  }
...
async function writeFunctionSymlink(outputDir, dest, fn, existingFunctions) {
  ...
  const targetDest = join2(outputDir, "functions", `${existingPath}.func`);
  const target = relative2(destDir, targetDest);
  await import_fs_extra3.default.mkdirp(destDir);
  await import_fs_extra3.default.symlink(target, dest);
  return true;
}
async function writeContainerImage(outputDir, containerImage, path2, functionConfiguration) {
  const dest = join2(outputDir, "functions", `${path2}.func`);
  ...
  await import_fs_extra3.default.mkdirp(dest);
  await import_fs_extra3.default.writeJSON(
    join2(dest, ".vc-config.json"),
    {
      handler,
      runtime: "container",
...
async function writeEdgeFunction(repoRootPath, outputDir, edgeFunction, path2, existingFunctions, standalone = false) {
  ...
  const configPath = join2(dest, ".vc-config.json");
  ops.push(
    import_fs_extra3.default.writeJSON(configPath, config, {
      spaces: 2
    })
  );
...
async function writeLambda(repoRootPath, outputDir, lambda, path2, functionConfiguration, existingFunctions, standalone = false) {
  ...
  for await (const dir of findDirs(".vercel", dest)) {
    const absDir = join2(dest, dir);
    const entries = await import_fs_extra3.default.readdir(absDir);
    if (entries.includes("cache")) {
      await Promise.all(
        entries.filter((e) => e !== "cache").map((entry) => import_fs_extra3.default.remove(join2(absDir, entry)))
      );
    } else {
      await import_fs_extra3.default.remove(absDir);
    }
  }
}
async function mergeBuilderOutput(outputDir, buildResult, workPath, rootOutputDir) {
  ...
  if (absOutputDir === buildResult.buildOutputPath) {
    const staticDir = join2(outputDir, "static");
    try {
...
async function relocateBuildOutputApiEntries(rootOutputDir, outputDir, workPath) {
  ...
  await Promise.all(
    BUILD_OUTPUT_API_ENTRIES.map(async (entry) => {
      const src = join2(rootOutputDir, entry);
      if (!await import_fs_extra3.default.pathExists(src))
        return;
      const dest = join2(outputDir, entry);
      if (entry === "static") {
...
async function cleanIgnoredFiles(dir, staticRoot, filter) {
  const entries = await import_fs_extra3.default.readdir(dir);
  await Promise.all(
    entries.map(async (entry) => {
      const entryPath = join2(dir, entry);
      const stat2 = await import_fs_extra3.default.stat(entryPath);
      const relativePath = relative2(staticRoot, entryPath);
      if (stat2.isDirectory()) {
        await cleanIgnoredFiles(entryPath, staticRoot, filter);
        const remaining = await import_fs_extra3.default.readdir(entryPath);
        if (remaining.length === 0) {
          await import_fs_extra3.default.rmdir(entryPath);
        }
      } else if (!filter(relativePath)) {
        output_manager_default.debug(`Removing ignored file: ${relativePath}`);
        await import_fs_extra3.default.remove(entryPath);
      }
    })
  );
}
...
async function* findDirs(name, dir, root = dir) {
  let paths;
  try {
    paths = await import_fs_extra3.default.readdir(dir);
  } catch (err) {
    if (err.code !== "ENOENT") {
      throw err;
    }
  ...
  for (const path2 of paths) {
    const abs = join2(dir, path2);
    let stat2;
    try {
      stat2 = await import_fs_extra3.default.lstat(abs);
    } catch (err) {
      if (err.code === "ENOENT")
        continue;
      throw err;
