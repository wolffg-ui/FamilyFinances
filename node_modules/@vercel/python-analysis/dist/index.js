...
import fs from "fs";
import { isAbsolute, join as join2, dirname as dirname2, resolve, sep } from "path";
...
async function readStatKind(path4) {
  try {
    const stats = await fs.promises.stat(path4);
    if (stats.isFile())
      return "file";
...
async function resolveNamePartsUncached(parts, searchDirs, cache) {
  const rel = join2(...parts);
  for (const root of searchDirs) {
    let target = null;
    let targetKind = null;
    const candidateInit = join2(root, rel, "__init__.py");
    if (await statKind(candidateInit, cache) === "file") {
      target = candidateInit;
      targetKind = "file";
    } else {
      const candidateModule = join2(root, `${rel}.py`);
      if (await statKind(candidateModule, cache) === "file") {
        target = candidateModule;
        targetKind = "file";
      } else if (await statKind(join2(root, rel), cache) === "dir") {
        target = join2(root, rel);
        targetKind = "dir";
      }
    }
    if (target !== null) {
      ...
      for (const part of parts.slice(0, -1)) {
        acc = join2(acc, part);
        const init = join2(acc, "__init__.py");
...
async function resolveImport(stmt, importerPath, roots, rootsKey, cache) {
  ...
  if (stmt.level > 0) {
    let base = dirname2(importerPath);
...
async function readDistInfoFile(distInfoDir, filename) {
  try {
    return await readFile2(join3(distInfoDir, filename), "utf-8");
  } catch {
    return void 0;
  }
}
async function scanDistributions(sitePackagesDir) {
  ...
  for (const dirName of distInfoDirs) {
    const distInfoPath = join3(sitePackagesDir, dirName);
...
async function extendDistRecord(sitePackagesDir, packageName, paths) {
  ...
  const recordPath = join3(sitePackagesDir, distInfoDirName, "RECORD");
  let existingRecord;
  try {
  ...
  if (newEntries.length > 0) {
    ...
    for (const p of newEntries) {
      const fullPath = join3(sitePackagesDir, p);
...
import { isAbsolute as isAbsolute2, join as join4, relative } from "path/posix";
