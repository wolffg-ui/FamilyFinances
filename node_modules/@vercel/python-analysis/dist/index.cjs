...
__export(src_exports, {
  ...
  collectImportClosure: () => collectImportClosure,
...
// src/semantic/import-graph.ts
var import_fs = __toESM(require("fs"), 1);
var import_path = require("path");
async function extractImports(source) {
...
var DEFAULT_MAX_FILES = 3e4;
async function readStatKind(path4) {
  try {
    const stats = await import_fs.default.promises.stat(path4);
    if (stats.isFile())
      return "file";
...
function statKind(path4, cache) {
  const cached = cache.statKinds.get(path4);
  if (cached)
    return cached;
  const pending = readStatKind(path4);
  cache.statKinds.set(path4, pending);
  return pending;
}
async function resolveNamePartsUncached(parts, searchDirs, cache) {
  const rel = (0, import_path.join)(...parts);
  for (const root of searchDirs) {
    let target = null;
    let targetKind = null;
    const candidateInit = (0, import_path.join)(root, rel, "__init__.py");
    if (await statKind(candidateInit, cache) === "file") {
      target = candidateInit;
      targetKind = "file";
    } else {
      const candidateModule = (0, import_path.join)(root, `${rel}.py`);
      if (await statKind(candidateModule, cache) === "file") {
        target = candidateModule;
        targetKind = "file";
      } else if (await statKind((0, import_path.join)(root, rel), cache) === "dir") {
        target = (0, import_path.join)(root, rel);
        targetKind = "dir";
      }
    }
    if (target !== null) {
      ...
      for (const part of parts.slice(0, -1)) {
        acc = (0, import_path.join)(acc, part);
        const init = (0, import_path.join)(acc, "__init__.py");
...
function resolveNameParts(parts, searchDirs, searchKey, cache) {
  if (parts.length === 0)
...
async function resolveSeedName(parts, searchDirs, searchKey, cache) {
...
function submoduleDir(target, targetKind) {
  if (targetKind === "dir")
    return target;
  if (targetKind === "file" && target.endsWith(`${import_path.sep}__init__.py`)) {
    return (0, import_path.dirname)(target);
  }
  return null;
}
async function resolveImport(stmt, importerPath, roots, rootsKey, cache) {
  const resolved = /* @__PURE__ */ new Set();
  let search = roots;
  let searchKey = rootsKey;
  if (stmt.level > 0) {
    let base = (0, import_path.dirname)(importerPath);
    for (let i = 1; i < stmt.level; i++) {
      base = (0, import_path.dirname)(base);
    }
    search = [base];
    searchKey = base;
  }
  ...
  if (subDir) {
    for (const name of stmt.names) {
      ...
      if (sub) {
        if (sub.targetKind === "file")
          resolved.add(sub.target);
...
function isModuleNameSeed(seed) {
  return !(0, import_path.isAbsolute)(seed) && /^[A-Za-z_][\w]*(\.[A-Za-z_][\w]*)*$/.test(seed);
}
async function collectImportClosure({
  ...
  const roots = searchRoots.map((r) => (0, import_path.resolve)(r));
  const rootsKey = roots.join("\0");
  const cache = {
    statKinds: /* @__PURE__ */ new Map(),
  ...
  for (const seed of seeds) {
    if (isModuleNameSeed(seed)) {
      ...
      if (name) {
        if (name.targetKind === "file")
          frontier.push(name.target);
        frontier.push(...name.parentChain);
      }
    } else {
      const seedPath = (0, import_path.resolve)(seed);
  ...
  while (frontier.length > 0) {
    ...
    const dependencies = await Promise.all(
      batch.map(async (importerPath) => {
        let source;
        try {
          source = await import_fs.default.promises.readFile(importerPath, "utf8");
        } catch {
          return [];
        }
        const stmts = await extractImports(source);
        const files = await Promise.all(
          stmts.filter((s) => s.isModuleLevel && !s.inTypeChecking).map((s) => resolveImport(s, importerPath, roots, rootsKey, cache))
        );
        return files.flat();
      })
    );
...
function hashFile(filePath) {
  return new Promise((resolve2, reject) => {
    ...
    stream.on("end", () => {
      resolve2({ hash: h.digest("base64url"), size });
    });
  });
}
